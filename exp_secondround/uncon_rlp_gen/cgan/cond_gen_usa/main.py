...
with open('exp_secondround/uncon_rlp_gen/cgan/config.yaml') as file:
    config = yaml.load(file, Loader=yaml.FullLoader)['USA_cgan']
...
# dataloader_test, _ = tl.create_data_loader(np_array_test, config['usa_cgan']['batch_size'], True)
...
# logging with wandb
wandb.init(project="cgan_usa", config=config)
...
for epoch in range(config['epochs']):
    ...
    if (epoch) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            plt.ylabel('Values')
            plt.savefig(f'exp_secondround/uncon_rlp_gen/cgan/cond_gen_usa/generated_samples.png')
            plt.close()
        
        # save the model
        torch.save(generator.state_dict(), f'exp_secondround/uncon_rlp_gen/cgan/cond_gen_usa/generator_usa.pth')
        
        # save the geneated samples
        fake_data_df = pd.DataFrame(fake_data)
        fake_data_df.to_csv(f'exp_secondround/uncon_rlp_gen/cgan/cond_gen_usa/generated_samples.csv', index=False)
