...
gen_data = gen_data[:, :-9]  # remove the last two columns which are conditions
...
original_data = np.array(original_data.iloc[:, 4:])
...
edis_vae = eva.calculate_energy_distances(original_data[:,:48], gen_data)
...
wdis_vae = eva.calculate_w_distances(original_data[:,:48], gen_data)
