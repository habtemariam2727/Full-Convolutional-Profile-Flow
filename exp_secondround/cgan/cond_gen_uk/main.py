...
for epoch in range(config['epochs']):
    ...
    if (epoch) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            plt.ylabel('Values')
            plt.savefig(f'exp_secondround/uncon_rlp_gen/cgan/cond_gen_uk/generated_samples.png')
            plt.close()
        
        # save the model
        torch.save(generator.state_dict(), f'exp_secondround/uncon_rlp_gen/cgan/cond_gen_uk/generator_uk.pth')
        
        # save the geneated samples
        fake_data_df = pd.DataFrame(fake_data)
        fake_data_df.to_csv(f'exp_secondround/uncon_rlp_gen/cgan/cond_gen_uk/generated_samples.csv', index=False)
