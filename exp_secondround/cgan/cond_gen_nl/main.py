...
for epoch in range(config['epochs']):
    ...
    if (epoch) % 20 == 0:
        ...
        # save the model
        torch.save(generator.state_dict(), f'exp_secondround/cgan/cond_gen_nl/generator_nl.pth')
        
        # save the geneated samples
        fake_data_df = pd.DataFrame(fake_data)
        fake_data_df.to_csv(f'exp_secondround/cgan/cond_gen_nl/generated_samples.csv', index=False)
