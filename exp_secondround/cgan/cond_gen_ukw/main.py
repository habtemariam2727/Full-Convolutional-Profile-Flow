...
dataloader_train, scaler = tl.create_data_loader(np_array_train[:300,:], config['batch_size'], True)
# dataloader_test, _ = tl.create_data_loader(np_array_test, config['ukw_cgan']['batch_size'], True)
...
# logging with wandb
wandb.init(project="cgan_ukw", config=config)
