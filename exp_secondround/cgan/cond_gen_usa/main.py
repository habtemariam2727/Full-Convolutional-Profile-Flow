...
for epoch in range(config['epochs']):
    ...
    if (epoch) % 20 == 0:
        ...
        with torch.no_grad():
            ...
            plt.ylabel('Values')
            plt.savefig(f'exp_secondround/cgan/cond_gen_usa/generated_samples.png')
            plt.close()
        
        # save the model
        torch.save(generator.state_dict(), f'exp_secondround/cgan/cond_gen_usa/generator_usa.pth')
        
        # save the geneated samples
        fake_data_df = pd.DataFrame(fake_data)
        fake_data_df.to_csv(f'exp_secondround/cgan/cond_gen_usa/generated_samples.csv', index=False)
